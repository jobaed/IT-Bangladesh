...
$contest = $conn->query("SELECT * FROM polytechnicteacher WHERE status = '1' ORDER BY id DESC");
?>
...
while ($row = $contest->fetch_assoc()) {
    ...
    echo "<td> <a href='poly-teacher-approve.php?id=" . $row['id'] . "' class='btn btn-primary'>Approve <i class='fa-solid fa-check'></i></a> <a href='delete_poly_teacher_addmission.php?id=" . $row['id'] . "' class='btn btn-danger mt-2'>Delete <i class='fa-solid fa-xmark'></i> </a> </td>";
    echo "</tr>";
